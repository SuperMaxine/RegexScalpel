...
//        String filePath = "data/old_dataset/large";
        String filePath = "data/paper_dataset";
        String fileName;
        fileName = "regexlib.txt";
        run(filePath + "/" + fileName, fileName);
    }

    private static void run(String s, String fileName) {
       run(s, fileName, "s", "java", "11111", 0, 15, 1, 60);
//        run(s, fileName, "s", "java", "00011", 0, 15, 1, 60);
//        run(s, fileName, "s", "java", "11100", 0, 15, 1, 60);
//         run(s, fileName, "s", "java", "10000", 0, 15, 1, 60);
//         run(s, fileName, "s", "java", "01000", 0, 15, 1, 60);
//         run(s, fileName, "s", "java", "00100", 0, 15, 1, 60);
//         run(s, fileName, "s", "java", "00010", 0, 15, 1, 60);
//         run(s, fileName, "s", "java", "00001", 0, 15, 1, 60);
    }
    ...
    public static void run(String sourceFile, String outfileName, String model, String language, String patternModel, int funcType, int checkThreadCount, int validateThreadCount, int timeout) {
        ...
        double averageTime = ((double) (checkEndTime - startTime) * (double) checkThreadCount + (double) (validateEndTime - checkEndTime) * (double) validateThreadCount) / (double) tasksData.size() / (double) 1000;
        ...
        list2.add("average time: " + averageTime + "(s)");
...
//            FileUtils.writeLines(new File("D:\\" + outfileName.replace(".txt", "") + "_redos-" + tsStr + ".txt"), list);
