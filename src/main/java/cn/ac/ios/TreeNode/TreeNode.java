...
import static cn.ac.ios.Patterns.POA.PatternPOAUtils.NO_LETTER_MATCH;
import static cn.ac.ios.TreeNode.Utils.*;
import static cn.ac.ios.Utils.BracketUtils.*;
import static cn.ac.ios.Utils.BracketUtils.getLetterSet;
...
public class TreeNode implements Iterable<TreeNode>, Serializable {
    ...
    private boolean isReferencesNode;
    ...
    public boolean isReferencesNode() {
        return isReferencesNode;
    }
...
//        return createReDoSTree(node.getData());
     ...
     * @param treeNode
     */
    private TreeNode letterReplace_Set(TreeNode treeNode) throws InterruptedException {
        String data = treeNode.getData();
        char c = data.charAt(0);
        if (Character.isLetter(c)) {
            if (Character.isLowerCase(c)) {
...
//        System.out.println(arrayList);
    ...
    public void escapeHexadecimal() throws InterruptedException {
        ...
        auxiliaryQueue.add(this);
        while (!auxiliaryQueue.isEmpty()) {
            TreeNode node = auxiliaryQueue.poll();
            if (isGroupNode(node)) {
                groupNodeCount++;
            }
            auxiliaryQueue.addAll(node.getChildList());
        }
        ...
        while (!queue.isEmpty()) {
            ...
            if (node.isLeaf()) {
                ...
                // 这里要改为先统计最大group数, 然后在group数内的\xxx即为反向引用 不修改 其他的均修改
                // 例如()()\1\2\3 这里group数是2 那么修改后变为()()\1\2\u0003 这种写法也只有js支持
                String back = nodeData.substring(1);
                if (Integer.parseInt(back) > groupNodeCount) {
                    back = Integer.toHexString(Integer.parseInt(back, 8));
                    if (back.length() == 1) {
                        nodeData = "\\u000" + back;
                    }
                    if (back.length() == 2) {
...
//                groupContentChainIndexList.add(new GroupContent(tree, tree.data, tree.chainIndex));
                ...
                groupContentChainIndexList.add(new GroupContent(groupSubNode, groupSubNode.data, groupSubNode.chainIndex));
            }
