...
import cn.ac.ios.EngineValidation.Java8.Trace;
import cn.ac.ios.EngineValidation.Java8.utils.NodeRelation;
import cn.ac.ios.EngineValidation.Java8.utils.PatternUtils;
import cn.ac.ios.EngineValidation.Java8.utils.RegexViewer;
...
public final class Pattern implements java.io.Serializable {
     ...
     *                                           invalid
     */
    public static Trace matches(String regex, CharSequence input, Trace trace) {
    ...
    public int getMatchingStepCnt(String prefix, String pump, String suffix, int max_length, int threshold) {
     ...
     * @return
     */
    public int getMatchingStepCnt(String prefix, String pump, String suffix, int max_length, int threshold, String validationFunction) {
        int repeat_cnt = (max_length - prefix.length() - suffix.length()) / pump.length();
        if (repeat_cnt < 1)
            return 0;
        String repeated = new String(new char[repeat_cnt]).replace("\0", pump);
        ...
        Matcher m = matcher(attack_string, new Trace(threshold, false));
        Trace t;
        if (validationFunction.equalsIgnoreCase("matches")) {
            t = m.matches();
        } else {
            t = m.find();
            return t.getMatchSteps();
