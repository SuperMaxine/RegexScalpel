...
public class Trace {
	public int threshold = 100000; // First judge: it's vulnerable when the number of match steps larger than CATASTROPHIC_THRESHOLD
...
//	private ArrayList<Integer> eachStep;
	...
	public Trace(int threshold, boolean isDetail) {
		this(threshold);
	...
	public boolean logMatch(Node node, int idx) {
		...
		this.eachStep[idx] ++;
//		while (this.eachStep.size() <= idx) {
//			this.eachStep.add(0);
//		}

//		int newStep = this.eachStep.get(idx) + 1;
//		this.eachStep.set(idx, newStep);
...
//		this.eachStep = new ArrayList<>();
		this.eachStep = new int[input.length() + 1];

//		System.out.println("this.eachStep.length = " + this.eachStep.length);
