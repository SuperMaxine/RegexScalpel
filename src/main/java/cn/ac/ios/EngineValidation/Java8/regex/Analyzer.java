package cn.ac.ios.EngineValidation.Java8.regex;

import com.alibaba.fastjson.JSONObject;
import org.javatuples.Quartet;
import org.javatuples.Triplet;
import cn.ac.ios.EngineValidation.Java8.regex.Pattern.Branch;
import cn.ac.ios.EngineValidation.Java8.regex.Pattern.Node;
import cn.ac.ios.EngineValidation.Java8.regex.Pattern.Ques;
import cn.ac.ios.EngineValidation.Java8.utils.PatternUtils;
...
public class Analyzer {
    ...
    private class VulStructure {
        ...
        Node curAtom;
        VulType type;
        Existence result = Existence.NOT_SURE;
        ...
        private class MatchGenerator {
            ...
            Map<Node, MatchGenerator> nextSliceSetMandatory = null;
            int min = 1;
            int max = 1;
            boolean isEnd = false;
            ...
            public void addMendatoryCase(Node node, MatchGenerator generator) {
                if (nextSliceSetMandatory == null)
                    nextSliceSetMandatory = new HashMap<Node, MatchGenerator>();
                nextSliceSetMandatory.put(node, generator);
        ...
        public class Driver {
            ...
            private boolean driverSatisfied() {
                if (reachFinal)
                    ...
                    if (newDriver.driverSatisfied())
                        ...
                        && curGenerator.nextSliceSetMandatory.containsKey(null)) {
                    Driver newDriver = new Driver(this);
                    newDriver.pushForward(null, curGenerator.nextSliceSetMandatory.get(null));
                    if (newDriver.driverSatisfied())
                        return true;
                }
                return false;
            ...
            public boolean hasNext(CurState state) {
                switch (state) {
                    ...
                    case ONLEAVE:
                        return curGenerator.nextSliceSetMandatory != null;
                }
                return false;
            }
            ...
            public void pushAny(CurState state) {
                switch (state) {
                    ...
                    case SATISFIED:
                        ...
                        break;
                    case ONLEAVE:
                        nodeSet = curGenerator.nextSliceSetMandatory.keySet();
                        nextSliceNode = nodeSet.iterator().next();
                        pushForward(nextSliceNode, curGenerator.nextSliceSetMandatory.get(nextSliceNode));
                        break;
                }
            }
            ...
            private void update(Map<Node, MatchGenerator> map, MatchGenerator startGenerator) {
                ...
                if (map.containsKey(null) && map.size() == 1) {
                    ...
                    for (Node node : map.keySet()) {
                        if (node == null && (!curGenerator.isEnd || curGenerator.nextSliceSetMandatory != map)) {
                            Driver newDriver = new Driver(this);
            ...
            public void getNextSlices() {
                if (getState() == CurState.UNSATISFIED && hasNext(CurState.UNSATISFIED)) {
                    ...
                    update(curGenerator.nextSliceSetMandatory, null);
                } else if (getState() == CurState.SATISFIED && hasNext(CurState.SATISFIED)) {
            ...
            public void getNextSlices(MatchGenerator startGenerator) {
                if (getState() == CurState.UNSATISFIED && hasNext(CurState.UNSATISFIED)) {
                    ...
                    update(curGenerator.nextSliceSetMandatory, startGenerator);
                } else if (getState() == CurState.SATISFIED && hasNext(CurState.SATISFIED)) {
            ...
            public void traverseToLast() {
                MatchGenerator coreGenerator = engine.headGenerator.nextSliceSetMandatory.values().iterator().next();
                ...
                for (Node node : curGenerator.nextSliceSetUnsatisfied.keySet()) {
                    ...
                    if (nextGen.isEnd && nextGen.nextSliceSetMandatory != null
                            && nextGen.nextSliceSetMandatory.containsValue(coreGenerator))
                        break;
                    Driver newDriver = new Driver(this);
                ...
                for (int i = 0; i < size; i++) {
                    ...
                    for (Node node : curDriver.curGenerator.nextSliceSetMandatory.keySet()) {
                        MatchGenerator nextGen = curDriver.curGenerator.nextSliceSetMandatory.get(node);
                        if (nextGen.isEnd && nextGen.nextSliceSetMandatory != null
                                && nextGen.nextSliceSetMandatory.containsValue(coreGenerator)) {
                            setAs(curDriver);
                        ...
                        newDriver.pushForward(node, curDriver.curGenerator.nextSliceSetMandatory.get(node));
                        genSet.add(newDriver);
                        size += 1;
                    }
                }
        ...
        private class DirectedEngine {
            ...
            public DirectedEngine(ArrayList<Node> sourcePath, boolean suffixSource) {
                ...
                do {
                    ...
                    if (index == directedPath.size() - 1) {
                        if (suffix)
                            tmpGenerator.nextSliceSetMandatory = null;
            ...
            private void addFromEnginePath(DirectedEngine nextEngine) {
                ...
                if (finalGenerator != null) {
                    finalGenerator.nextSliceSetMandatory = null;
            ...
            private void addFromEngine(DirectedEngine nextEngine) {
                Map<Node, MatchGenerator> map = nextEngine.headGenerator.nextSliceSetMandatory;
                if (map == null) {
            ...
            private boolean notFinish(MatchGenerator generator, int cnt) {
                return !generator.isUnsatisfied(cnt) && generator.nextSliceSetMandatory == null
            ...
            private MatchGenerator buildGenerators(Node node, MatchGenerator lastGeneratorTmp, boolean sub,
                ...
                if (node.sub_next != null) {
                    ...
                    if (node.sub_next == next_node) {
                        nextGenerator.nextSliceSetMandatory = nextGenerator.nextSliceSetSatisfied;
                        nextGenerator.min = 0;
                    }
                }
        ...
        private Set<Driver> getNewOption(Map<Driver, Quartet<Driver, Node, MatchGenerator, Set<Integer>>> optionMap,
            ...
            for (Driver driver : optionMap.keySet()) {
                ...
                while (newDriver.getState() == CurState.ONLEAVE
                        && newDriver.curGenerator.nextSliceSetMandatory.size() == 1
                        && newDriver.curGenerator.nextSliceSetMandatory.containsKey(null))
                    newDriver.pushForward(null, newDriver.curGenerator.nextSliceSetMandatory.get(null));
                if (str == null)
                    return null;
                else if (str != "") {
                    if (sliceRemain == null) {
        ...
        private boolean pushSliceToSatisfied(Driver driver, String str) {
            while (driver.getState() == CurState.ONLEAVE && driver.curGenerator.nextSliceSetMandatory.size() == 1
                    && driver.curGenerator.nextSliceSetMandatory.containsKey(null))
                driver.pushForward(null, driver.curGenerator.nextSliceSetMandatory.get(null));
            driver.getNextSlices();
        ...
        public void printResult(BufferedWriter outVul, int index) throws IOException {
            ...
            if (result != Existence.NOT_EXIST) {
                if (!possible_vulnerability) {
                    outVul.write(regex + "\n");
                    possible_vulnerability = true;
                }
                if (result == Existence.EXIST)
    ...
    public void doDynamicAnalysis(BufferedWriter outVul, int index, int threshold) throws IOException {
        ...
        for (VulStructure vulCase : possibleVuls) {
            ...
            if (vulCase.result == Existence.EXIST) {
    ...
    private boolean checkResult(String prefix, String pump, String suffix, int maxLength, int threshold) {
