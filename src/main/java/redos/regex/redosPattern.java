...
public final class redosPattern implements java.io.Serializable {
    ...
    static class SliceNode extends Node {
        ...
        public Vector<Set<Integer>> getSliceSet(){
    ...
    static class BnM extends Node {
        ...
        public Vector<Set<Integer>> getSliceSets() {
            Vector<Set<Integer>> result = new Vector<>();
            for (int b : buffer) {
                Set<Integer> tmp = new HashSet<>();
                tmp.add(b);
    ...
    public List<String> getAllSlices() {
        if (nodes.isEmpty())
            traverseAllNodes();
        List<String> slices = new Vector<String>();
        for (Node n : nodes) {
     ...
     * 需要在外面手动替换ch内容
     */
    public Vector<Set<Integer>> checkSet(Node node, Set<Integer> ch){
        if (node instanceof SliceNode) {
            Vector<Set<Integer>> str = ((SliceNode) node).getSliceSet();
            // Todo： 对Dot的支持
            Set<Integer> firstCh = str.get(0);
            firstCh.retainAll(ch);
            if (firstCh.size() != 0)
                // return (Vector<Set<Integer>>) str.subList(1, str.size());
                return str;
            else
                return null;
        } else if (node instanceof BnM) {
            Vector<Set<Integer>> str = ((BnM) node).getSliceSets();
            // Todo： 对Dot的支持
            Set<Integer> firstCh = str.get(0);
            firstCh.retainAll(ch);
            if (firstCh.size() != 0)
            ...
            charCh.retainAll(ch);
            if(charCh.size() != 0){
                Vector<Set<Integer>> result = new Vector<Set<Integer>>();
                result.add(charCh);
                return result;
            }
            else
    ...
    public Vector<Set<Integer>> getSliceSets(Node node) {
        Vector<Set<Integer>> result = new Vector<>();
        ...
        if (prev == null) {
            ...
            if (!visitedNodes.containsKey(cur)) {
                ...
                if (!visitedEdges.containsKey(prev)) {
                    ...
                    Vector<Node> tos = new Vector<Node>();
                    tos.add(cur);
