...
loop:   for(int x=0, offset=0; x<nCodePoints; x++, offset+=len) {
    ...
    LOOP:
        for (;;) {
            ...
            case '[':
                ...
                node = clazz(true);
                endIndex = cursor;
                node.setRegex(temp, beginIndex, endIndex);
                break;
            case '\\':
                beginIndex = cursor;
                ch = nextEscaped();
    ...
    private Node group0() {
        ...
        if (node instanceof Ques) {
            ...
            head.setRegex(regexTemp, beginIndex, endIndex);
            root = tail;
            return head;
        } else if (node instanceof Curly) {
            ...
            if (head.study(info)) { // Deterministic
                ...
                loop.setRegex(regexTemp, beginIndex, endIndex);
    ...
    public static class Node extends Object {
        ...
        Vector<Vector<Set<Integer>>> Path;
        public String regex = "";
        public void setRegex(int[] temp, int begin, int end) {
            regex = "";
...
NEXT:       for (i = patternLength; i > 0; i--) {
            ...
            if (node instanceof SliceS)
                return new BnMS(src, lastOcc, optoSft, node.next, node.regex);
            return new BnM(src, lastOcc, optoSft, node.next, node.regex);
        }
        BnM(int[] src, int[] lastOcc, int[] optoSft, Node next, String regex) {
            this.buffer = src;
...
NEXT:       while (i <= last) {
    ...
    static final class BnMS extends BnM {
        ...
        BnMS(int[] src, int[] lastOcc, int[] optoSft, Node next, String regex) {
            super(src, lastOcc, optoSft, next, regex);
